...
class Single_Input_Baseline:
    ...
    def quarter_2_iso8601(self, df_series):
        ...
        if quarter == 1:
            return datetime.strptime(f"{year}-03-31", "%Y-%m-%d").isoformat()
        if quarter == 2:
            return datetime.strptime(f"{year}-06-30", "%Y-%m-%d").isoformat()
        if quarter == 3:
            return datetime.strptime(f"{year}-09-30", "%Y-%m-%d").isoformat()
        if quarter == 4:
            return datetime.strptime(f"{year}-12-31", "%Y-%m-%d").isoformat()
        raise TypeError

    def single_input(self):
        df = read_gold_data()
        df = df[["county_fips", "year", "quarter", "period_id", "total_est_attendance", "taxable_sales_usd"]]
        df["record-identier"] = df["county_fips"] + "-" + df["period_id"]
        df["event-time"] = df[["year", "quarter"]].apply(self.quarter_2_iso8601, axis=1)
        return df[["record-identier", "event-time", "total_est_attendance", "taxable_sales_usd"]]
